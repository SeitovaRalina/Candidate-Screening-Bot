...
### §3.1. Terse output never breaks structured output
Terseness (the `.claude/terse/` ruleset applied to all subagents, including via the `terse-inject.sh` hook) governs **free prose only**. §3 structured-findings blocks (YAML/JSON) are exempt: emit every required key with its exact key name, and write the full value-field text verbatim — never drop, abbreviate, or fragment keys or enum values (severity / confidence / category enums, file paths, line numbers, `true`/`false`), and never collapse the list structure into a paragraph. This ratifies the terse-by-default change against §3.
...
Feature branches → PR → review → merge. **Carve-out (D-044):** the harness *source* repo's code owner (per `CODEOWNERS`) may push directly to `main` — an authorized human exercising ownership, not agent auto-merge (which stays forbidden, ANTI-3). This carve-out is source-repo-only; it does not license an agent to push to a consuming project's main. Destructive git ops (`reset --hard`, `push --force`, `branch -D`) require explicit user confirmation. Hooks bypass (`--no-verify`, `--no-gpg-sign`) forbidden unless user asks verbatim.
