...
Parse the plan's **Task container** header FIRST — it is the portable, handoff-ready brief. Honor it as binding scope:
- `scope-fence` — only touch files/dirs listed in-scope; everything else is out.
- `definition-of-done` — the checklist the implementation must satisfy.
- `verification-contract` — the exact commands / gates to run in Step 5 (the verify gate).
- `inputs-needed` — env/creds the executor must have (names only, §12); if a required input is absent, surface it before fanning out.
- `out-of-scope` — hard exclusions; do not implement opportunistically.
...
- **Visual critic is MANDATORY (not optional) when the diff touches a UI surface** — any changed file matching `*.tsx` / `*.jsx` / `*.vue` / `*.svelte` / `*.swift` / `*.css` / `components/`, or a project that declares a UI surface. Before the implementor may report `status: complete`, the verify gate MUST:
  1. Invoke an existing critic skill — `apple-design-critic` (Apple targets), or `visual-spec` (web-frontend module) / the `frontend` consilium role (web). Do not hand-roll a parallel review.
