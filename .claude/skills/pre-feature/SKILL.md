...
- **Task container** (write FIRST, at the top of the report) — a portable, handoff-ready brief so the plan hands cleanly agent→human and human→agent. Fixed header block:
  ```
  ## Task container
  - slug: <kebab>
  - scope-fence: <files/dirs in-scope; everything else is out>
  - definition-of-done: <checklist, each item independently verifiable>
  - verification-contract: <exact commands / gates that prove DoD, e.g. `make test`, p95 threshold>
  - inputs-needed: <secrets/env/creds the executor must have — names only, §12>
