...
## Tools
- hh.ru vacancy: **official public API** (`api.hh.ru/vacancies/{id}`), read-only, no login — **this is currently blocked** (verified live 2026-09-04: hh.ru returns 403 for any unauthorized request since an April 2026 policy change, see D-9/OQ-5). Code path exists and works once/if an hh.ru API app is registered; until then, pasted vacancy text is the actually-working MVP path, not a degraded fallback.
